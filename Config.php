...
class Wave_Config {
    ...
    public static function get($file){
		...
		if (!isset($configs[$file])){
			if(0 === strpos($file, 'routes/')){
				// if in development then generate the routes every time
				...
				if($routes == null)
					throw new Wave_Exception('Could not load routes for domain: '.$baseurl.' nor default domain: '.$defaultdomain);
				else
					$configs[$file] = $routes;
