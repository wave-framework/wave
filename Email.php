...
**/
class Wave_Email extends Swift_Message {
	...
	public static function create(){		
		...
		if(!self::$_isbuilt){
			...
			if(self::$config->transport === 'SMTP'){
				self::$transport = Swift_SmtpTransport::newInstance($config->server, $config->port);
				if(self::$config->auth)
					self::$transport->setUsername($config->username)
